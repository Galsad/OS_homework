...
int process_arglist(int count, char** arglist){
    ...
    if (sigaction(SIGINT, &ignore_signal, NULL) != 0){
        write(stderr,"error initializing signal\n\0",27);
        exit(1);
    }
    ...
    if (count <= 0){
        write(stderr,"non positives amount of commands!\n",30);
        return 1;
    }
